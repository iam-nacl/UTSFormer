...
class PreNorm_tokendict(nn.Module):
    def __init__(self, dim, fn):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(dim)
        self.fn = fn
    def forward(self, q_dict, kv_dict, **kwargs):
        q_dict['x'] = self.norm1(q_dict['x'])
        kv_dict['x'] = self.norm2(kv_dict['x'])
        return self.fn(q_dict, kv_dict, **kwargs)

class FeedForward_tokendict(nn.Module):
    def __init__(self, dim, hidden_dim, dropout=0.):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(dim, hidden_dim),
...
class Attention_saveAttn(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
...
class Attention_UTS_attnloss(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
...
class UTSTransformer(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_UTS(
            dim,
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
...
class UTSTransformer_changePreNorm(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.layers.append(nn.ModuleList([
            PreNormHW(dim, Attention_UTS(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)),
            PreNorm(dim, FeedForward(dim, mlp_dim, dropout=drop))]))

    def forward(self, x, H, W):

        for attn, ff in self.layers:
            attn_x, attn_map = attn(x, H, W)
            x = x + attn_x
            x = ff(x) + x
        return x, attn_map

class UTSTransformer_attnloss(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_UTS_attnloss(
            dim,
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
...
class ATMAttention_attnloss(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, use_sr_layer=True):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
...
class ATMAttention_attnloss_tokenscore(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, use_sr_layer=True):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
...
class UTSBlock_changePreNorm(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, use_sr_layer=True):
        super().__init__()
        self.norm1 = norm_layer(dim)
        ...
        self.layers = nn.ModuleList([])
        self.layers.append(nn.ModuleList([
            PreNorm_tokendict(dim, ATMAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
                                       attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)),
            PreNorm(dim, FeedForward_tokendict(dim, mlp_dim, dropout=drop))]))
...
class UTSBlock_attnloss(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, use_sr_layer=True):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = ATMAttention_attnloss(
            dim,
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio, use_sr_layer=use_sr_layer)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
    ...
    def forward(self, inputs):
        ...
        # attn
        attnx, attn = self.attn(q_dict, kv_dict)
        x = x + self.drop_path(attnx)
...
class UTSBlock_attnloss_tokenscore(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, use_sr_layer=True):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = ATMAttention_attnloss_tokenscore(
            dim,
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio, use_sr_layer=use_sr_layer)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
...
class HTM(nn.Module):
    ...
    def forward(self, token_dict, token_dicts, as_out, ctm_stage):
        token_dict = token_dict.copy()
        x = self.conv(token_dict)
        x = self.norm(x)
        token_score1 = self.score(x)
        token_weight1 = token_score1.exp()  # [b, 1024, 1]

        token_dict['x'] = x
        B, N, C = x.shape

        # token_dict['token_score'] = token_score1
        token_dict['token_score'] = torch.zeros(token_score1.shape).cuda()
        ...
        if ctm_stage == 1:
            idx_cluster, cluster_num, token_weight2, token_weight3, variance, score = FDGCCS(
                token_dict, as_out, cluster_num, self.sample_ratio, ctm_stage, self.k)
        elif ctm_stage == 2:
            idx_cluster, cluster_num, token_weight2, token_weight3, variance, score = ADDCCS1(
                token_dict, as_out, cluster_num, r2, ctm_stage, self.k)
            variance = var_downup(variance, token_dicts[0], token_dicts[1])
        elif ctm_stage == 3:
            idx_cluster, cluster_num, token_weight2, token_weight3, variance, score = ADGCCS2(
                token_dict, as_out, cluster_num, r3, ctm_stage, self.k)
            variance = var_downup(variance, token_dicts[1], token_dicts[2])
            variance = var_downup(variance, token_dicts[0], token_dicts[1])
        ...
        epsilon = 1e-8
        token_weight1_sum = token_weight1.sum(dim=1, keepdim=True)
        token_weight1_sum = token_weight1_sum + (token_weight1_sum == 0).float() * epsilon
        token_weight1 = token_weight1 / token_weight1_sum
        token_weight2_sum = token_weight2.sum(dim=1, keepdim=True)
        token_weight2_sum = token_weight2_sum + (token_weight2_sum == 0).float() * epsilon
        token_weight2 = token_weight2 / token_weight2_sum
        token_weight3_sum = token_weight3.sum(dim=1, keepdim=True)
        token_weight3_sum = token_weight3_sum + (token_weight3_sum == 0).float() * epsilon
        token_weight3 = token_weight3 / token_weight3_sum

        token_weight = (token_weight1 + token_weight2 + token_weight3)/3
        ...
        return [down_dict, token_dict], token_weight, idx_cluster, variance, score
...
class FAT_PTRV2(nn.Module):
    def __init__(self,
        ...
        for i in range(self.start_level, self.end_level + 1):  #start_level=0, end_level=3 (range结果不包括4)
            l_conv = nn.Conv2d(in_channels=in_channels[i], out_channels=out_channels, kernel_size = 1).cuda()
            self.lateral_convs.append(l_conv)

        for i in range(self.start_level, self.end_level):  # start_level=0, end_level=3 (range结果不包括3)
    ...
    def forward(self, inputs):
        """Forward function."""
        assert len(inputs) == len(self.in_channels) #为了保证输入一定有4个

        # build lateral tokens
        input_dicts = []
        for i, lateral_conv in enumerate(self.lateral_convs):  # 4个ConvModule,分别对inputs4个进行卷积,结果存在input_dicts里
            tmp = inputs[i + self.start_level].copy()  # start_level=0
            tmp['x'] = lateral_conv(tmp['x'].unsqueeze(2).permute(0, 3, 1, 2)).permute(0, 2, 3, 1).squeeze(2)
            input_dicts.append(tmp)

        # merge from high level to low level
        for i in range(len(input_dicts) - 2, -1, -1):  # i = 2,1,0
            input_dicts[i]['x'] = input_dicts[i]['x'] + token_downup(input_dicts[i], input_dicts[i + 1])
            input_dicts[i]['x'] = self.merge_blocks[i](input_dicts[i])
...
def FDGCCS(token_dict, as_out, cluster_num, sample_ratio, ctm_stage, k=5, token_mask=None):
    with torch.no_grad():
        x = token_dict['x']
        B, N, C = x.shape
        ...
        token_weight2 = attn.sum(dim=1).unsqueeze(dim=-1)
        ...
        variance = torch.var(dist_nearest, dim=2, keepdim=True)
        ...
        token_weight3,_ = dist_matrix.min(dim=1)
        max_values = token_weight3.max(dim=1, keepdim=True).values
        token_weight3 = (max_values - token_weight3).unsqueeze(dim=-1)
    ...
    return idx_cluster, cluster_num, token_weight2, token_weight3, variance, score


def ADDCCS1(token_dict, as_out, cluster_num, r2, ctm_stage, k=5, token_mask=None):
    with torch.no_grad():
        x = token_dict['x']
        B, N, C = x.shape
        dist_matrix1 = torch.cdist(x, x) / (C ** 0.5)
        ...
        attn = attn_matrixs[0].sum(dim=1)
        token_weight2 = attn.sum(dim=1).unsqueeze(dim=-1)
        token_weight2 = token_weight2/(token_weight2+0.000001)
        ...
        # min_vals = variance.min(dim=1, keepdim=True)[0]
        # max_vals = variance.max(dim=1, keepdim=True)[0]
        # normalized_variance = (variance - min_vals) / (max_vals - min_vals + 1e-8)


        useFirstScore = torch.zeros(weight_as.shape, device=weight_as.device)
        # useTokenNum = int(cluster_num) # 完全由Stage决定
        # useTokenNum = int(cluster_num*0.5) # 一半由Stage决定
        useTokenNum = int(cluster_num*r2) #
        useFirstScore[:, :useTokenNum] = torch.arange(useTokenNum, 0, -1).unsqueeze(0)


        # # select clustering center according to score
        score = weight_as + useFirstScore # weightasOnly
        # score = normalized_variance.squeeze(dim=-1) /ctm_stage + weight_as + useFirstScore #Plus
        # score = dist * normalized_variance.squeeze(dim=-1) / ctm_stage + weight_as + useFirstScore  # PlusDist
        # score = 0.5 * dist * density + weight_as + useFirstScore #PlusDPC0.5
        # score = 0.2 * dist * density + weight_as + useFirstScore #PlusDPC0.2
...
def ADGCCS2(token_dict, as_out, cluster_num, r3, ctm_stage, k=5, token_mask=None):
    with torch.no_grad():
        x = token_dict['x']
        B, N, C = x.shape
        dist_matrix1 = torch.cdist(x, x) / (C ** 0.5)
        ...
        # useTokenNum = int(cluster_num) # 完全由Stage决定
        # useTokenNum = int(cluster_num * 0.5)  # 一半由Stage决定
        useTokenNum = int(cluster_num * r3)  # 一半由Stage决定
        useFirstScore[:, :useTokenNum] = torch.arange(useTokenNum, 0, -1).unsqueeze(0)
...
def merge_labelas(token_dict, idx_cluster, cluster_num, token_weight=None):
    """Merge tokens in the same cluster to a single cluster.
    Implemented by torch.index_add(). Flops: B*N*(C+2)
    Return:
        out_dict (dict): dict for output token information
...
def var_downup(variance, target_dict, source_dict):
    """
    Transform variance between different distributions based on target and source token relationships.

    Args:
        variance (Tensor[B, S, 1]): Source token variance tensor.
        target_dict (dict): Target token information dictionary.
        source_dict (dict): Source token information dictionary.

    Returns:
        var_out (Tensor[B, T, 1]): Transformed variance tensor for target tokens.
    """

    # Source and target token information
    x_s = source_dict['x']  # Shape: [B, S, C]
    idx_token_s = source_dict['idx_token']
    idx_token_t = target_dict['idx_token']
    T = target_dict['token_num']
    B, S, C = x_s.shape
    ...
    # Aggregation weight from target_dict or default to ones
    weight = target_dict['agg_weight'] if 'agg_weight' in target_dict.keys() else None
    if weight is None:
        weight = x_s.new_ones(B, N_init, 1)
    weight = weight.reshape(-1)

    # Adjust variance shape for matrix multiplication
    variance = variance.reshape(B, S, 1)

    # Select approach based on computation cost
    if N_init < T * S:
        # Use sparse matrix multiplication
        idx_token_t = idx_token_t + torch.arange(B, device=x_s.device)[:, None] * T
        idx_token_s = idx_token_s + torch.arange(B, device=x_s.device)[:, None] * S
        ...
        # Use fp32 for torch.sparse operations
        with torch.cuda.amp.autocast(enabled=False):
            weight = weight.float().detach()
            A = torch.sparse.FloatTensor(coor, weight, torch.Size([B * T, B * S]))
            all_weight = A.type(torch.float32) @ variance.new_ones(B * S, 1).type(torch.float32) + 1e-6
            weight = weight / all_weight[(idx_token_t).reshape(-1), 0]
            A = torch.sparse.FloatTensor(coor, weight, torch.Size([B * T, B * S]))
            var_out = A.type(torch.float32) @ variance.reshape(B * S, 1).type(torch.float32)
    else:
        # Use dense matrix multiplication
        idx_batch = torch.arange(B, device=x_s.device)[:, None].expand(B, N_init)
        coor = torch.stack([idx_batch, idx_token_t, idx_token_s], dim=0).reshape(3, B * N_init)
    ...
    # Reshape var_out to [B, T, 1] and return
    var_out = var_out.reshape(B, T, 1).type(variance.dtype)
    return var_out

def attn_downup(variance, target_dict, source_dict):
    """
    Transform variance between different distributions based on target and source token relationships.
...
class Block_saveAttn(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_saveAttn(
            dim,
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
