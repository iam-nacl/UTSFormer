...
class PixelEmbed(nn.Module):
    ...
    def forward(self, x, pixel_pos):
        B, C, H, W = x.shape
        assert H == self.img_size[0], f"Input image height ({H}) doesn't match model height ({self.img_size[0]})."
        assert W == self.img_size[1], f"Input image width ({W}) doesn't match model width ({self.img_size[1]})."
        x = self.proj(x)
        x = self.unfold(x)
