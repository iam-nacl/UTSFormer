...
class ResNet_BasicBlock_OS8(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            try:
                resnet.load_state_dict(torch.load("./models/CNN/pretrained_models/resnet18-5c106cde.pth"))
            except FileNotFoundError:
                print("Model file not found. Automatically downloading the model...")
                model_url = "https://download.pytorch.org/models/resnet18-5c106cde.pth"
                torch.hub.download_url_to_file(model_url, "./models/CNN/pretrained_models/resnet18-5c106cde.pth")
            ...
            resnet = models.resnet34()
            # load pretrained model:
            resnet.load_state_dict(torch.load("./models/CNN/pretrained_models/resnet34-333f7ec4.pth"))
            # remove fully connected layer, avg pool, layer4 and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-4])
