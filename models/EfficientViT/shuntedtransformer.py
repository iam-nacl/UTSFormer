...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., h=16, w=16):
        super().__init__()
    ...
    def forward(self, x):
        x = self.fc1(x)
        x = self.act(x + self.dwconv(x, self.h, self.w))
        x = self.drop(x)
        x = self.fc2(x)
        x = self.drop(x)
        return x
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=4, h=16, w=16):
        super().__init__()
...
class Transformer_shunted(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim=1024, dropout=0., num_patches=128):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Attention(dim, num_heads=heads, h=H, w=W)),
                #PreNorm(dim, Mlp(in_features=dim, hidden_features=mlp_dim, drop=dropout, h=H, w=W)),
