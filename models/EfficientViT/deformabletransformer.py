...
class DAttentionBaseline(nn.Module):
    ...
    def __init__(
        self, q_patch_size=(16, 16), kv_window_size=7, n_heads=12, n_head_channels=768, n_groups=1,
        ...
        self.q_h, self.q_w = q_patch_size
    ...
    def forward(self, x):
        B, N, c = x.shape
        x = x.reshape(B, self.q_h, self.q_w, c).permute(0, 3, 1, 2)
        B, C, H, W = x.size()
        dtype, device = x.dtype, x.device

        q = self.proj_q(x) # [b c h w] -> [b c h w]
        q_off = einops.rearrange(q, 'b (g c) h w -> (b g) c h w', g=self.n_groups, c=self.n_group_channels) #[b g h w]
...
class Transformer_da(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim=1024, dropout=0., num_patches=128):
        ...
        self.h, self.w = int(math.sqrt(num_patches)), int(math.sqrt(num_patches))
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, DAttentionBaseline(q_patch_size=(self.h, self.w), n_head_channels=dim//heads, n_heads=heads)),
                PreNorm(dim, Mlp(in_features=dim, hidden_features=mlp_dim, drop=dropout))
            ]))
    def forward(self, x):
