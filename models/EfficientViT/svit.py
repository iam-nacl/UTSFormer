...
class StokenAttention(nn.Module):
    ...
    def stoken_forward(self, x):
        ...
        pixel_features = rearrange(pixel_features, 'b d h w -> b (h w) d')
...
class Transformer_sta(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim=1024, dropout=0., num_patches=128):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, StokenAttention(dim, num_heads=heads, stoken_size=to_2tuple(4))),
